...
| M4 | 后端底座 + 8 平台模块 + 契约驱动 | 🔄 worktree 并行中 |
| M5 | canonical reference（business-notice/order/approval） | ⏸ 等 M4 完成 |
| M6 | 验证层完整版 + 主题样本库 | ⏸ 待开始 |
| M7 | 开源准备 | ⏸ 待开始 |
...
**当前规则**（5 pitfall + 2 playbook，按场景分类见 INDEX.md）：
...
| `cross-review-residual-scan.md` | pitfall | 流程 | 批量替换后 grep 旧名称归零 + 多 agent 并行后必须交叉审查 |
...
- **日期**: 2026-04-14
- **内容**:
  1. **M3 前端完整实现**：L3 八大业务组件（55 tests）+ L4 应用壳层（i18n/theme/auth/menu/layouts/error/feedback）+ L5 TanStack Router 文件路由 + api-sdk HTTP 客户端
  2. **L2 升级为 shadcn/ui v4 原版**：19 个 @radix-ui 分包 → radix-ui 统一包，Toast → Sonner
  3. **L1 补 8 个 sidebar color token**：TOTAL_TOKENS 46 → 54
  4. **质量门禁扩展到 12 项**：新增 check:i18n + check:business-words + dep-cruiser M3 规则
- **上一次**: 2026-04-14（M1 脚手架实施完成）
- **下一次**: M5 前后端汇合时
