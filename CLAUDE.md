...
| **M2** | 前端 L1 主题系统 + L2 原子组件（30 个 shadcn/ui v4） | ✅ 已完成 |
| **M3** | 前端 L3 业务组件 + L4 壳层 + L5 路由 + api-sdk | ✅ 已完成 |
| **M4** | 后端底座 + 8 平台模块 + 契约驱动 | ✅ 已完成 |
...
│   │   ├── ui-primitives/           # L2 原子组件（30 个 shadcn/ui v4 原版 + Sonner）
...
**当前规则**（6 pitfall + 2 playbook，按场景分类见 INDEX.md）：
...
| `cross-review-residual-scan.md` | pitfall | 流程 | 批量替换后 grep 旧名称归零（含文档）+ 多 agent 并行后必须交叉审查 |
| `plan-code-snippets-must-verify.md` | pitfall | 流程 | 计划中的方法名/类名必须 grep 验证，不能凭记忆写 |
...
- **日期**: 2026-04-14
- **内容**:
  1. **M4 后端底座完整实现**：7 infra 模块 + 8 platform 模块 + 18 Flyway DDL + 24 ArchUnit 规则 + 46 测试
  2. **认证体系**：Sa-Token JWT Simple + RefreshTokenService（Redis rotation）+ SaInterceptor 全局认证（opt-out 安全模型）
  3. **数据权限**：DataScopeVisitListener（ExecuteListener）+ DataScopeRegistry 集中声明 + BypassDataScopeAspect
  4. **5 轮审查驱动修复**：逐阶段 review + Codex 对抗性审查 + 4 角色并行审查（安全/AI执行者/架构师/编译验证）
- **上一次**: 2026-04-14（M3 前端完整实现）
- **下一次**: M5 canonical reference
