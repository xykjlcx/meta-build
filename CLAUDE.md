...
| **Plan A** | Claude Design 对齐 · 基础设施（claude-warm style + 3 claude-* preset + HeaderTab） | ✅ 已完成（2026-04-19） |
...
| **Infra 层改动（`mb-common` / `mb-schema` / `mb-infra/**` / `application*.yml` / `pom.xml` 根配置）** | **先写 ADR（或 mini-ADR 链接到 issue）再实施**。理由：底层改动"看似小修"会波及多层消费者（参考 meta-0023 Actuator 端口 / CompositeTaskDecorator 试错），ADR 的写作过程是"是不是必须做"的自检强制摩擦 |
| 要改 CLAUDE.md | **只改顶层导航和指针**，不要塞架构细节（详情放 specs） |
| 发现文档-代码 drift | **立即修正**，不允许"等下次一起改"——nxboot 的反面教材 |
| **收到根因未知的调查类请求** | **先调查 → 结构化汇报（现象 / 假设 / 证据 / 方案选项） → 等我确认方案 → 再执行**。不要边调查边修、不要给第一眼结论下就行动（meta-0023 会话 openapi 500 调查因此 3 次改口）|
| 每完成一个 milestone | 回补 `[M1 时补]` / `[M4 时补]` 占位 + 更新本文档"当前阶段"表格 + 规则库健康度检查 |
| 写项目日志 | 末尾必须包含"规则库复盘"区域（见下方模板） |
| 承接交接文档（新会话继承上个会话的 TODO 清单） | 走 [`docs/handoff/meta-review-template.md`](docs/handoff/meta-review-template.md)：每条任务答 "是不是真问题" 4 问后分类 🔴立即做 / 🟡登记到 pre-prod / 🔵撤销。未经 meta-review 的清单不直接执行 |
| 批次级任务结束（≥3 commit / infra 层 / 契约变更 / ADR 撤销） | 主动做二轮 review（不等我问），按 [`docs/rules/second-pass-review.md`](docs/rules/second-pass-review.md) 的 6 维度清单 |
...
**当前规则**（10 pitfall + 2 playbook，按场景分类见 INDEX.md）：
...
| `tdd-enforcement.md` | pitfall | 流程/全栈 | 业务逻辑代码必须 TDD：先写失败测试再实现，测试 commit 先于实现 commit；骨架/纯视觉/spike 例外 |
...
cd client && pnpm check:theme                             # 主题完整性（3 style × 2 mode × 70 token）
...
cd client && pnpm check:deps                              # 依赖方向检查（9 条规则）
...
- **日期**: 2026-04-19
- **内容**: **Plan A · Claude Design 对齐基础设施**（feat/claude-design-plan-a → main，27 commits）
  1. **L1 Token**：新增 `claude-warm` style（light + dark 各 70 core token）+ DESIGN 文档，现有 `classic` / `lark-console` 保留不动；default style 切到 `claude-warm`
  2. **密度系统**：`ThemeScale` 类型 xs/lg → compact/comfortable，含 localStorage + DOM dataset 双向 migration helper + 4 个新单测；数值对齐 Claude Design（36/44/52 px）
  3. **L4 共享组件**：`HeaderTabSwitcher`（aria tablist + 键盘导航 + overflow 下拉）、`useActiveModule`（URL → localStorage → fallback 三级 + SSR 守卫）、`SystemSwitcherPopover`（9 格 × Google Apps 视觉 + disabled Tooltip）、`BgBlobs`（两个模糊圆形渐变）
  4. **3 Preset**：`claude-classic` / `claude-inset`（bg-blobs + 毛玻璃 topbar + 主体内嵌白卡片）/ `claude-rail`（窄 w-16 图标侧栏），100% 还原 Claude Design `.layout--classic/inset/rail`；`supportedDimensions: []`（形态差异通过切 preset 表达）
  5. **默认 preset** 从 `inset` 切到 `claude-inset`；`SystemItem` / `ShellLayoutProps.systems` slot 前置 Task 11 定义，避免 app-shell 跨层 import web-admin；inset / mix 也接入九宫格
  6. **ADR**：frontend-0024（14 决策汇总）+ frontend-0025（三层导航哲学 + 3 段"拒绝的替代方案"）；新 rule `l3-sedimentation-principle.md`
  7. **30 组合 Playwright visual regression baseline**：3 style × 5 preset × 2 ColorMode 自动截图 + CI 比对
  8. **2 轮 Review**：code-reviewer 对抗审查（v1 发现 4 Critical + 11 Warning，v2 修订后再 review 发现 5 新 Critical 全部闭环）→ "概念 7 实施真相源"纪律
  9. **质量**：70 vitest + 56 Playwright E2E（含 30 visual baseline）全绿；9 条 depcruise 规则 0 violations
- **上一次**: 2026-04-15（M5 business-notice / SSE / 4 渠道通知 / Notice 前端 + UI 打磨）
- **下一次**: Plan B（IAM 3 页重写 + Mock 菜单重组 3 Tab）或 Plan C（Dashboard / Auth / Profile / 错误页）—— 见 [plan-a-complete.md](docs/handoff/claude-design-alignment/plan-a-complete.md) §6
