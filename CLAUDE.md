...
- **日期**: 2026-04-14
- **内容**:
  1. **M1 脚手架实施完成**：后端 6 层 Maven（18 模块）+ 前端 5 层 pnpm workspace（6 包 + web-admin）+ Docker + CI
  2. **jOOQ codegen 切换到官方三插件模式**：groovy-maven-plugin → flyway-maven-plugin → jooq-codegen-maven
  3. **Docker 端口规范**：所有中间件映射端口加 1 前缀（PG 15432, Redis 16379）
  4. **CLAUDE.md 更新**：M1 状态标记已完成 + 常用命令填充
- **上一次**: 2026-04-14（项目规则库建立 + CLAUDE.md 规则库段落）
- **下一次**: M2 开始时
