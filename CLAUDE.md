...
│   │   ├── 0007-继承遗产前先问原生哲学.md
│   │   ├── 0008-flyway-migration命名用时间戳.md
│   │   ├── 0009-表前缀sys改mb.md
│   │   ├── 0010-审计日志改操作日志.md
│   │   ├── 0011-version字段按需添加.md
...
| 数据 | TanStack Query |
| UI | Radix UI + Tailwind CSS v4 + CVA + shadcn 风格（源码在 `@mb/ui-primitives`） |
...
项目级经验沉淀，每次 session 自动加载。从项目日志中提炼的具体踩坑经验，格式：**规则 → Why（事件+日期） → How to apply**。

| 规则文件 | 一句话 |
|---------|-------|
| `adr-before-code.md` | 翻转决策必须先写 ADR 再改代码，不允许补写 |
| `verify-block-discipline.md` | specs 新增约束必须同步新增 verify 块 + 扩展 verify-docs.sh |
| `template-propagation-risk.md` | 模板 bug 会被 AI 在 N 个业务模块中批量复制，修复成本 O(n) |
| `cross-review-residual-scan.md` | 批量替换后 grep 旧名称归零 + 多 agent 并行后必须交叉审查 |
| `frontend-backend-joint-check.md` | 改了 API 契约相关内容必须联查对端 specs |
| `multi-perspective-review.md` | 大型变更用多视角 agent 并行审查，单一视角永远有盲区 |

**维护约定**：
- 做事前扫一遍相关 rules，避免重复踩坑
- 做事后发现新坑主动写入，不等用户要求
- 写 rules 的标准：能防止未来同类错误的具体经验，不是泛泛的原则
...
| [.claude/rules/](.claude/rules/) | **项目踩坑规则库**（自动加载，持续积累） | 持续更新 |
...
- **日期**: 2026-04-13
- **内容**:
  1. **前端架构文档全套定稿**：`docs/specs/frontend/` 12 子文件 + README 反向索引 + appendix（M0 前端部分完成）
  2. **CLAUDE.md 一致性修复**：M0 状态从"前端未写"更新为"已定稿"；前端阅读路径、技术栈表头、前端 MUST/MUST NOT 小节、文档索引表全部指向 `frontend/README.md`
  3. **技术栈修正**：Tailwind CSS 3.4 → v4
  4. **后端 README 文档边界修正**：`frontend-architecture.md` → `frontend/README.md`
- **上一次**: 2026-04-11（后端 specs 定稿 + N3/C2/M4.x 系列落盘，详见 git log）
- **下一次**: M1 脚手架完成时
