...
使用 React Context（与 ThemeProvider 同构）：
...
**包裹关系**：`LayoutResolver` 内部先渲染 `LayoutPresetProvider`，Provider 的 children 中再用 `useLayoutPreset()` 读取 presetId 并渲染对应 Layout。具体实现：LayoutResolver 分为外层（包 Provider）和内层（消费 Context）两个组件。
...
function LayoutResolver({ children, notificationSlot }: Props) {
  return (
    <LayoutPresetProvider>
      <LayoutResolverInner notificationSlot={notificationSlot}>
        {children}
      </LayoutResolverInner>
    </LayoutPresetProvider>
  )
...
// 内层组件，在 Provider 内部才能消费 Context
function LayoutResolverInner({ children, notificationSlot }: Props) {
...
**sideEffects 配置**：`app-shell/package.json` 需要声明 `"sideEffects": ["./src/layouts/presets/**/index.ts"]`，防止 bundler 误删 side-effect import。
...
**DOM 属性迁移**：`data-theme="default|dark|compact"` → `data-style="classic"` + `data-mode="dark"`

**localStorage key 迁移**：旧 key `mb-theme` → 新 key `mb_style` + `mb_color_mode`。StyleProvider 初始化时需读取旧 key 做兼容迁移：

```ts
// 兼容读取逻辑（StyleProvider 初始化时执行一次）
function migrateOldThemeKey(): void {
  const old = localStorage.getItem('mb-theme')
  if (!old) return
  if (old === 'dark') {
  ...
  localStorage.removeItem('mb-theme')  // 迁移后清除旧 key
}
```

**`apply-theme.ts` 处理**：该文件（`ui-tokens/src/apply-theme.ts`）包含 `applyTheme/loadTheme/initTheme/isValidTheme` 四个函数，被 `main.tsx` 在 React mount 前同步调用。迁移方案：
- **删除** `apply-theme.ts`（功能被 StyleProvider + 防闪烁脚本取代）
- **`main.tsx`** 中移除 `import { initTheme } from '@mb/ui-tokens'` 和 `initTheme()` 调用
- 防闪烁由 `index.html` 内联脚本接管（Section 5.4）
- StyleProvider 负责运行时的 DOM 属性管理

**受影响的消费方完整清单**（16 个文件）：

| 文件 | 当前用法 | 迁移 |
|------|---------|------|
| `ui-tokens/src/apply-theme.ts` | 核心运行时 | **删除** |
| `ui-tokens/src/theme-registry.ts` | ThemeId + themeRegistry | **删除**（被 style-registry.ts 替代） |
| `ui-tokens/src/index.ts` | 导出 applyTheme/loadTheme/initTheme/themeRegistry | **改**：移除旧导出，新增 styleRegistry 导出 |
| `app-shell/src/theme/theme-provider.tsx` | ThemeProvider + ThemeContext | **删除**（被 style-provider.tsx 替代） |
| `app-shell/src/theme/use-theme.ts` | useTheme hook | **删除**（被 use-style.ts 替代） |
| `app-shell/src/theme/index.ts` | 导出 ThemeProvider/useTheme | **改**：导出 StyleProvider/useStyle |
| `app-shell/src/components/theme-switcher.tsx` | ThemeSwitcher 组件 | **删除** |
| `app-shell/src/index.ts` | 导出 ThemeProvider/useTheme/SidebarLayout/TopLayout | **改**：更新导出 |
| `web-admin/src/main.tsx` | initTheme() + ThemeProvider | **改**：移除 initTheme，ThemeProvider → StyleProvider |
| `web-admin/src/styles.css` | @import themes/index.css | **改**：@import styles/index.css |
| `ui-primitives/.storybook/preview.tsx` | dataset.theme = theme | **改**：dataset.style + dataset.mode |
| `ui-patterns/.storybook/preview.tsx` | themeRegistry + dataset.theme | **改**：styleRegistry + dataset.style + dataset.mode |
| `ui-tokens/scripts/check-theme-integrity.ts` | 遍历 themeRegistry | **改**：遍历 styleRegistry，按选择器块分别提取变量 |
| `themes/default.css` | [data-theme='default'] | **删除**（合并到 styles/classic.css） |
| `themes/dark.css` | [data-theme='dark'] | **删除**（合并到 styles/classic.css） |
| `themes/compact.css` | [data-theme='compact'] | **删除**（归入 Scale XS） |
...
| `styles/classic.css` | 新建 | 从 default.css + dark.css 合并，选择器改 [data-style][data-mode] |
...
| `src/index.ts` | 改 | 移除 applyTheme/loadTheme/initTheme/themeRegistry 导出，新增 styleRegistry 导出，新增 radius 基准值 |
| `src/theme-registry.ts` | 删 | 被 style-registry.ts 替代 |
| `src/apply-theme.ts` | 删 | 功能被 StyleProvider + 防闪烁脚本取代 |
| `src/style-registry.ts` | 新建 | StylePresetDef + StyleRegistry |
| `package.json` | 改 | exports 更新：`"./themes/*"` → `"./styles/*"`，新增 `"./customizer.css"` |
| `scripts/check-theme-integrity.ts` | 改 | 遍历 styleRegistry，按选择器块分别提取 light+dark 变量 |
...
| **theme/ 重构** | | |
| `theme/index.ts` | 改 | 导出 StyleProvider/useStyle 替代 ThemeProvider/useTheme |
| **废弃文件** | | |
...
| `components/top-nav.tsx` | 删 | 仅被 TopLayout 使用，一并删除 |
| `layouts/sidebar-layout.tsx` | 删 | 被 LayoutResolver + presets 替代 |
| `layouts/top-layout.tsx` | 删 | 未使用，删除 |
| `src/index.ts` | 改 | 更新导出：ThemeProvider→StyleProvider, useTheme→useStyle, SidebarLayout/TopLayout→LayoutResolver |
| `package.json` | 改 | 新增 exports：`./layouts/presets/inset`、`./layouts/presets/module-switcher`、`./customizer`；新增 `sideEffects` 配置 |

### L5 web-admin

| 文件 | 动作 | 说明 |
|------|------|------|
| `src/main.tsx` | 改 | 移除 `initTheme()` 调用 + `ThemeProvider` → `StyleProvider` |
...
| `index.html` | 改 | `<div id="root">` 前插入防闪烁内联脚本 |

### Storybook 配置

| 文件 | 动作 | 说明 |
|------|------|------|
| `ui-primitives/.storybook/preview.tsx` | 改 | `dataset.theme` → `dataset.style` + `dataset.mode`，硬编码主题项改为 styleRegistry |
| `ui-patterns/.storybook/preview.tsx` | 改 | 同上 |
...
- 新建：~25 文件
- 删除：~12 文件
- 修改：~14 文件（含 main.tsx、package.json exports、Storybook、check-theme 等）
- **L2/L3 组件改动：0**
...
1. **菜单不导航**：MenuNode 没有 path 字段，**所有布局**的菜单项暂时纯展示，不做路由跳转。Module Switcher 的 Tab 切换只更新 Sidebar 内容，不自动导航。Inset 布局的 Sidebar 菜单项也不导航。path 字段作为独立增强单独排期。
...
4. **布局切换无过渡动画**：两种布局 DOM 结构差异极大，切换为硬切（整个组件树替换），不做过渡动画。
5. **Breadcrumb 数据源**：Inset Header 预留 Breadcrumb 位置，但数据源（菜单树 + 当前路径匹配）依赖 MenuNode.path，本次先显示静态文本或不显示。
6. **Module Switcher 单模块场景**：如果菜单树只有 1 个 depth 0 模块，Header Tab 区域隐藏（直接展示 Sidebar，退化为类似 Inset 的体验）。
...
两种布局的 Header 工具栏（右侧）统一排列：

```
LanguageSwitcher | ThemeCustomizer(齿轮) | NotificationBadge(铃铛) | Separator | UserAvatar
```

- **LanguageSwitcher**：保留现有组件，从废弃的 `header.tsx` 迁移到各 preset 的 header 中
- **ThemeCustomizer**：替代 ThemeSwitcher，齿轮图标
- **NotificationBadge**：通过 `notificationSlot` prop 由 L5 注入

### Content Layout（Full/Centered）实现

```css
/* centered 模式：在各 layout preset 的内容区 div 上条件应用 */
[data-theme-content-layout=centered] .content-wrapper {
  max-width: var(--size-content-max-width);  /* 80rem，已有 token */
  margin-inline: auto;
}
```

各 layout preset 的内容区 div 需要加 `content-wrapper` class（或用 Tailwind group-data 内联方式）。Full 模式为默认，不需要额外样式。

### ThemeCustomizer 移动端

移动端（< 768px）DropdownMenu 可能溢出。处理方式：
- DropdownMenuContent 加 `max-h-[80vh] overflow-y-auto`
- 布局切换和内容宽度控件在移动端隐藏（`hidden lg:flex`），与 shadcnuikit 一致
