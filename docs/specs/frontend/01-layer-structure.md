...
| L2 | `@mb/ui-primitives` | 30 个原子组件（shadcn/Radix 风格）| Radix UI / shadcn primitives / CVA / clsx |
| L3 | `@mb/ui-patterns` | 7 个业务组件（NxTable / NxForm / ...）| TanStack Table / React Hook Form / Zod / date-fns |
...
| ★ | `@mb/api-sdk` | OpenAPI Generator 产物（不入 git） | OpenAPI Generator 客户端运行时 |
...
│       ├── tsconfig.json
│       └── src/
│           ├── generated/                     # OpenAPI Generator 产物（不入 git）
...
| 定制深度 | 任意改 L1-L5 源码（包括底座的 30 个原子组件） | 通过 props / theme override / shadcn 式复制源码 |
...
| L2 `@mb/ui-primitives` | shadcn/ui CLI 拷贝 + meta-build 调整 | 30 个原子组件，参考 shadcn 默认实现 |
| L3 `@mb/ui-patterns` | meta-build 自写 | 7 个业务组件全部新写，参考 nxboot 同类组件 |
...
| `@mb/api-sdk` | OpenAPI Generator 自动生成 | 不入 git，构建时生成 |
...
**`@mb/api-sdk`**：
- OpenAPI Generator 客户端运行时（具体包取决于 generator 选择，例如 `@hey-api/client-fetch` 或 `axios`）
- 无 `@mb/*` 依赖
