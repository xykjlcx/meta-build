...
3. **最小可用默认实现**：
   ...
   - 30 个原子组件（[03 §3](./03-ui-primitives.md#3-30-个原子组件清单)）
...
**改的对象**：30 个原子组件中的任何一个的实现（CVA variants / ref 转发行为 / 默认 props / shadcn 风格的内部 JSX）。
...
5. **跑 Storybook 看效果** `pnpm -C client/packages/ui-primitives storybook`——所有 30 个原子组件会立刻显示新颜色
...
| 30 个原子组件清单 + Storybook 规范 | [03-ui-primitives.md](./03-ui-primitives.md) |
