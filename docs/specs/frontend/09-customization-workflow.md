...
3. **最小可用默认实现**：
   - 1 套参考主题 `default`（[02 §6.1](./02-ui-tokens-theme.md#61-default-主题中性基调参考主题)）
   - 29 个原子组件（[03 §3](./03-ui-primitives.md#3-29-个原子组件清单)）
   - 7 个业务组件（[04 §3](./04-ui-patterns.md#3-7-个核心业务组件清单)）
   - 3 套布局预设（[05 §3](./05-app-shell.md#3-布局预设-m3)）
...
| 布局调整 | 改 `SidebarLayout.tsx` 的 JSX 结构 | 在 L4 里 `import { orderApi } from '@mb/api-sdk'`（违反白名单依赖） |
...
| 2 | L2 | 原子组件（Button / Input / Dialog / 等 29 个） | `client/packages/ui-primitives/src/{button,input,dialog}.tsx` | **中** | TS strict + Storybook 故事 + Vitest 单测 + L2 i18n 禁用规则 | 所有上层使用者（L3 / L4 / L5） |
...
**改的对象**：29 个原子组件中的任何一个的实现（CVA variants / ref 转发行为 / 默认 props / shadcn 风格的内部 JSX）。
...
**改的对象**：NxTable / NxForm / NxFilter / NxDrawer / NxBar / NxLoading / ApiSelect 7 个业务组件之一的内部实现。
...
├── layouts/
│   ├── sidebar-layout.tsx     # 侧边栏 + 内容区布局
│   ├── top-layout.tsx         # 顶部导航布局
│   └── basic-layout.tsx       # 无菜单布局
├── providers/
│   └── app-providers.tsx      # 6 层 Provider 树
├── header/
│   └── header.tsx             # 顶部 Header
├── sidebar/
│   └── sidebar.tsx            # 侧边栏（消费 useMenu）
└── i18n/
...
- `useMenu()` 怎么从后端 sys_menu 树拉数据（[07 §8](./07-menu-permission.md#8-前端-usemenu-hook)）
- 主题 Provider 怎么和 `data-theme` 属性切换协作（[02 §5.1](./02-ui-tokens-theme.md#51-data-theme-属性切换)）
...
1. **打开默认主题文件** `client/packages/ui-tokens/src/themes/default.css`
2. **修改颜色变量**——找到 `--color-primary` 和 `--color-primary-foreground` 两行，改成新的颜色值
3. **同步修改暗色主题** `client/packages/ui-tokens/src/themes/dark.css` 的对应变量
4. **跑主题完整性校验** `pnpm -C client check:theme`，确认每个主题都有完整的 46 个 token
5. **跑 Storybook 看效果** `pnpm -C client/packages/ui-primitives storybook`——所有 29 个原子组件会立刻显示新颜色
...
约束：
1. 只改 client/packages/ui-tokens/src/themes/default.css 和 dark.css 的 --color-primary 和 --color-primary-foreground
...
- 不要在 `default.css` 之外的任何地方写蓝色 hex code——否则定制就被污染了
...
- 如果想加新 size variant（如 "compact"），CVA 的类型会自动推导，不用手改 props 接口
...
2. **理解原结构**——侧边栏消费 `useMenu()` 拿到 sys_menu 树（详见 [07 §8](./07-menu-permission.md#8-前端-usemenu-hook)），递归渲染每一级菜单
3. **改渲染逻辑**——把递归的 `<MenuTree>` 改成两段渲染：
   - 一级菜单（`level === 0`）渲染为 `<SidebarGroup>` 标题
   - 二级菜单（`level === 1`）渲染为 `<SidebarItem>` 列表项
4. **不要改 useMenu / sys_menu 数据结构**——只改渲染层
...
**预期影响**：所有用 `SidebarLayout` 的页面侧边栏外观变化。其他布局（`TopLayout` / `BasicLayout`）不受影响。
...
7. 后端权限点必须先在 sys_route_tree 注册（详见 docs/specs/frontend/07-menu-permission.md §6）

请：
1. 列出要创建的所有文件路径
2. 给出每个文件的完整代码（不要 // TODO 占位）
3. 列出运维需要在菜单管理页做哪些事（添加 sys_menu 节点 / 给角色赋权）
...
**前提**：使用者已经定义了 default / dark / compact 三套主题（[02 §6](./02-ui-tokens-theme.md#6-初始-3-套主题-m2)）。

**步骤**：

1. 启动 dev server：`pnpm -C client/apps/web-admin dev`
2. 登录到任意业务页面（例如订单列表）
3. 在 Header 右上角点击 ThemeSwitcher，依次切换 default / dark / compact
...
3. 在 Theme Registry 注册新主题
4. 启动 dev server，在 ThemeSwitcher 选 feishu
5. **断言**：
   - 所有页面的主色变成飞书蓝
...
| 29 个原子组件清单 + Storybook 规范 | [03-ui-primitives.md](./03-ui-primitives.md) |
| 7 个业务组件 + L3 不硬编码业务语义 | [04-ui-patterns.md](./04-ui-patterns.md) |
