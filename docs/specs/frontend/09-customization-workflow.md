...
3. **最小可用默认实现**：
   - 1 套 canonical style `classic`（[02 §6.1](./02-ui-tokens-theme.md#61-canonical-styleclassic)）
...
**预期影响**：所有使用 `inset` preset 的页面侧边栏外观变化。`module-switcher` 和 `BasicLayout` 不受影响。
...
2. 登录到任意业务页面（例如订单列表）
3. 在 Header 右上角点击 ThemeCustomizer，依次切换 `classic light` / `classic dark` / `scale=xs + radius=sm`
