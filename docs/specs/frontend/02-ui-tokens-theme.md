...
> **关注点**：L1 `@mb/ui-tokens` + 纯 CSS Variables Only 哲学 + 54 个核心语义 token + 扁平命名 + Style Registry + ColorMode + Customizer CSS 维度 + 主题完整性校验脚本 + Tailwind v4 CSS-first 配置 + 使用者扩展新风格。
>
...
| Token 总数 | 54 个语义 token，分 6 组（colors / radii / sizes / shadows / motion / fonts） |
...
| `--color-sidebar-accent` | 侧边栏 hover 背景 | `var(--color-gray-200)` |
| `--color-sidebar-accent-foreground` | 侧边栏 hover 文字 | `var(--color-gray-950)` |
...
@import '../tokens/semantic-feishu.css';
...
**`TOTAL_TOKENS` 守护**：`index.ts` 中导出的 `TOTAL_TOKENS = 54` 作为唯一事实源，脚本加载 `TOKEN_NAMES` 后断言长度等于 `TOTAL_TOKENS`，防止 token 新增后常量未更新的 drift。
...
使用者想加一套新风格（例如”飞书风格”）需要 **4 步**（其中步骤 4 是 M6 前的临时双维护方案）：

### 步骤 1：定义 semantic token 覆写

在 `packages/ui-tokens/src/tokens/semantic-<id>.css` 创建新文件，参考 `semantic-classic.css` 的结构，按需覆写 54 × 2 mode 的 token：
...
```bash
cd client
pnpm -F @mb/ui-tokens check:theme       # 主题完整性校验（54 × 2 mode token 全量覆盖）
```
