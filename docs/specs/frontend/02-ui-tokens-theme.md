...
| 初始风格数量 | **3 套 style**：`classic`（canonical）/ `lark-console`（飞书管理后台风格）/ `claude-warm`（Claude Design 对齐，Plan A 新增）；浅色/深色由 `ColorMode` 切换，`compact` 废弃并迁移到 Customizer 组合 |
...
| `scale` | `<body data-theme-scale="compact">` | `mb_scale` | 密度 / 字号 / 基础间距（合法值：`compact` / `default` / `comfortable`） |
...
| `compact` | `mb_style=classic` + `mb_scale=compact` + `mb_radius=sm` |
...
- `compact` → `classic + scale=compact + radius=sm`
...
@import '../tokens/semantic-claude-warm.css';
@import '../tokens/component.css';
```

每新增一套 style（如 `semantic-my-brand.css`），就在最后一个 `semantic-*.css` 之后追加一行 `@import`（保持 `primitive → semantic-* → component` 顺序）。
