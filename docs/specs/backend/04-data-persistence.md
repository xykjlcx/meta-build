...
#### 禁止的方式（ArchUnit `NO_DIRECT_TIME_API` 规则守护）
...
public static final ArchRule NO_DIRECT_TIME_API = noClasses()
    .that().resideInAPackage("..domain..")
    .should(new ArchCondition<JavaClass>("not call Instant.now() / OffsetDateTime.now() without Clock") {
        @Override
        public void check(JavaClass clazz, ConditionEvents events) {
            clazz.getMethodCallsFromSelf().stream()
                .filter(call -> isDirectTimeCall(call))
                .forEach(call -> events.add(SimpleConditionEvent.violated(clazz,
                    clazz.getName() + " 直接调用了无参时间 API，必须通过 Clock Bean 获取时间")));
        }

        private boolean isDirectTimeCall(JavaMethodCall call) {
            String owner = call.getTargetOwner().getFullName();
            String name = call.getName();
            return (owner.equals("java.time.Instant") && name.equals("now") && call.getTarget().getParameterTypes().isEmpty())
                || (owner.equals("java.time.OffsetDateTime") && name.equals("now") && call.getTarget().getParameterTypes().isEmpty())
                || (owner.equals("java.time.LocalDateTime") && name.equals("now") && call.getTarget().getParameterTypes().isEmpty())
                || owner.equals("java.lang.System") && name.equals("currentTimeMillis");
        }
    })
    .as("Domain 层禁止直接调用无参时间 API，必须通过注入的 Clock Bean 获取时间");
```
