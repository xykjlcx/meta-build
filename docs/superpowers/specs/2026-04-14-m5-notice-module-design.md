...
| 12 | 批量操作 | 前端预检 + 后端校验双保险，返回 `BatchResultVo` |
...
| 模块 | 生成内容 |
|------|---------|
| `notice` | 全部 hooks（useGetNotices、useCreateNotice、usePublishNotice 等）+ 全部 DTO（NoticeVo、NoticeCreateCmd、BatchResultVo 等） |
...
│   │   └── ...
│   ├── models/           # TypeScript 接口（NoticeVo, NoticeCreateCmd 等）
│   │   └── index.ts
...
**复制为新建**：`POST /api/v1/notices/{id}/duplicate`，从已发布/已撤回公告复制标题、正文、附件关联，生成新的 DRAFT 公告，返回 `NoticeVo`。
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/v1/notices` | `notice:notice:list` | 分页列表 → `PageResult<NoticeVo>` |
| GET | `/api/v1/notices/{id}` | `notice:notice:detail` | 详情（含附件）→ `NoticeDetailVo` |
| POST | `/api/v1/notices` | `notice:notice:create` | 新增草稿 → 201 + `NoticeVo` |
| PUT | `/api/v1/notices/{id}` | `notice:notice:update` | 编辑（**仅 DRAFT 状态**）→ `NoticeVo` |
| DELETE | `/api/v1/notices/{id}` | `notice:notice:delete` | 删除（仅 DRAFT/REVOKED）→ 204 |
| POST | `/api/v1/notices/{id}/publish` | `notice:notice:publish` | 发布（含 targets 参数）→ `NoticeVo` |
| POST | `/api/v1/notices/{id}/revoke` | `notice:notice:publish` | 撤回 → `NoticeVo` |
| POST | `/api/v1/notices/{id}/duplicate` | `notice:notice:create` | 复制为新建草稿 → 201 + `NoticeVo` |
| POST | `/api/v1/notices/batch-publish` | `notice:notice:publish` | 批量发布 → `BatchResultVo` |
| DELETE | `/api/v1/notices/batch` | `notice:notice:delete` | 批量删除 → `BatchResultVo` |
...
| GET | `/api/v1/notices/{id}/recipients` | `notice:notice:detail` | 已读人员列表 → `PageResult<RecipientVo>` |
| GET | `/api/v1/notification-logs` | `notice:notice:detail` | 通知发送记录 → `PageResult<NotificationLogVo>` |
...
| GET | `/api/v1/wechat/bindStatus` | 已登录即可 | 当前用户绑定状态 → `WechatBindStatusVo` |

**已发布不可编辑**：`PUT /api/v1/notices/{id}` 只允许 DRAFT 状态，已发布或已撤回调编辑返回 400 `notice.onlyDraftCanEdit`。

**批量操作约束**：`ids` 数组长度上限 100，超出返回 400。批量操作返回 `BatchResultVo { success, skipped }`，后端跳过不合规项（如批量发布时跳过非 DRAFT 项）。
...
```java
// 列表视图
public record NoticeVo(Long id, String title, int status, boolean pinned,
...
public record NoticeDetailVo(Long id, String title, String content, int status,
    boolean pinned, Instant startTime, Instant endTime,
    List<AttachmentVo> attachments, List<NoticeTargetVo> targets,
...
// 通知发送记录视图
public record NotificationLogVo(Long id, String channelType, Long recipientId,
    String recipientName, int status, String errorMessage, Instant sentAt) {}

// 新增
public record NoticeCreateCmd(
...
public record BatchResultVo(int success, int skipped) {}

// 查询条件
public record NoticeQry(Integer status, String keyword,
...
// 微信绑定状态
public record WechatBindStatusVo(boolean mpBound, boolean miniBound,
...
| 删除已撤回 | 204 |
| 删除已发布 | 400 |
| 批量发布 | 返回 `BatchResultVo`，非 DRAFT 项被 skip |
| 批量删除 | 返回 `BatchResultVo`，非 DRAFT/REVOKED 项被 skip |
...
5. 后端仍做校验（防绕过），返回 `BatchResultVo { success, skipped }`
6. 前端刷新列表
...
| 删除草稿 | 删除→确认→消失 |
| 删除已撤回 | 删除→确认→消失 |
| 批量发布 | 勾选→弹确认框（含跳过提示）→确认→BatchResultVo |
| 批量删除 | 勾选→弹确认框→确认→BatchResultVo |
| 导出 | 导出→下载 xlsx |
...
Phase 2: Notice 后端
  ...
  ├── Controller（含已读/未读端点 + 批量操作 BatchResultVo + 已发布只读校验）
  ├── 复制为新建 API（duplicate）
