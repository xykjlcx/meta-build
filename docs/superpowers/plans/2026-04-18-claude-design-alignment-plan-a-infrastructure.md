...
**Architecture:** Token 层新增 `claude-warm` style（70 个 core semantic token × light/dark 两种 ColorMode），Layout Registry 新增 3 个 preset（`claude-classic / claude-inset / claude-rail`），共享 `HeaderTabSwitcher` 组件把顶层菜单节点渲染为 Tab，`useActiveModule` hook 从 URL 推导当前激活模块。零数据模型改动（后端 `mb_iam_menu` 不动）。
...
| | classic | lark-console | **claude-warm**（默认） |
|---|---|---|---|
...
### 概念 7：实施真相源（强制 verify 锚点，严守 plan-code-snippets-must-verify）

Plan 初稿和 v1 修订都**凭空假设了若干 API / 路径 / 命名**导致 code-review 发现多个 Critical。为防止再犯，本 Plan 的每个 code snippet 都锚定到以下真相源，subagent 执行前**必须先读对应文件**验证自己的理解，再按 Plan 的指令落地。

| 主题 | 真相源文件 | 关键事实 |
|---|---|---|
| Token 总数 | `client/packages/ui-tokens/src/index.ts:97` | `export const TOTAL_TOKENS = 70`——**每个 style 在 light + dark 两种 ColorMode 下都各提供 70 个 core semantic token**（通过 `TOKEN_NAMES` 白名单过滤）。CSS 文件可以写更多行（注释 / 扩展变量），但 core token 必须齐 70 |
| check:theme 机制 | `client/packages/ui-tokens/scripts/check-theme-integrity.ts` | 以 `classic.light` 为基准，比对每个 style block = 70；新增 `claude-warm` 后验证集变为 3 × 2 = 6 个 block，每块 70 |
| Style 注册位置 | `client/packages/ui-tokens/src/style-registry.ts:103-119` | 注册在本文件末尾（不是 main.tsx）。调用签名 `styleRegistry.register({ id, displayName, description, color: '#xxx', cssFile: './tokens/semantic-<id>.css' })`——color 是纯 hex，cssFile 是相对 `src/` 的路径 |
| 密度 token 命名 | `client/packages/ui-tokens/src/customizer.css` | 实际 token 是 `--size-control-height / --size-header-height / --size-sidebar-width / --size-sidebar-width-collapsed`（**不是** Claude Design 的 `--row-h / --pad-x / --gap / --font-size-base`）。Plan A 策略：保持命名只调数值对齐 Claude Design 体感 |
| 默认 style 机制 | `client/packages/app-shell/src/theme/style-provider.tsx:74-113` 和 `normalizeStyleId` | **没有** `DEFAULT_STYLE_ID` 常量；默认值来自 `StyleProvider` 的 `defaultStyle` prop（默认 `'classic'`）+ `normalizeStyleId` 的 `'classic'` 兜底 |
| LegacyTheme 映射 | `client/packages/app-shell/src/theme/style-provider.tsx:85-93` | `mapLegacyThemeToState('compact')` 第 89 行 `scale: 'xs'`——Task 8 改类型后需要同步改为 `'compact'` |
| Migration 插入点 | `client/packages/app-shell/src/theme/style-provider.tsx:116-141` (`readStateFromStorage`) + `143-163` (`readStateFromDom`) | 两处都要同步处理 `xs → compact / lg → comfortable` 迁移 |
| menu 工具 re-exports | `client/packages/app-shell/src/menu/index.ts` | 只 re-export `useMenu / resolveMenuIcon / MenuNode / UserMenuPayload`。`isDisplayNode / getDisplayChildren / findMenuPathByPath / MenuHrefResolver` 必须从 `menu/menu-utils` **直接** 导入 |
| 现有 preset 参考 | `client/packages/app-shell/src/presets/inset/inset-layout.tsx` 第 57-63 行 + `presets/mix/mix-layout.tsx` 第 43-50 行 | 正确的 menu 工具 import 范式。Plan A 新 preset 保持一致 |
| depcruise 规则真相 | `client/.dependency-cruiser.cjs` | 9 条规则（不是 7）。关键守护：`l4-app-shell-no-l5`、`l3-no-forbidden-deps`、`features-no-api-sdk-auth` |
| Claude Design 视觉源 | `/Users/ocean/Desktop/claude-design-test01/styles/tokens.css` + `styles/app.css` + `components/shell.jsx` | Plan A 的视觉最终目标是 **100% 还原 Claude Design**。preset 的 DOM 结构、className、density 数值全部以此为准，**不** 因为"shadcn/ui 有 Sidebar 复合组件"而绕路 |

**subagent 纪律**：执行每个 Task 前先 `cat` 对应真相源文件的指定行区间，如果 Plan 的指令和真相源有偏差，**以真相源为准**并回来修 Plan。禁止凭 Plan 的 snippet 猜测 API。
...
├── tokens/
│   └── semantic-claude-warm.css                           [新建：70 × 2 ColorMode，结构对齐 semantic-classic.css]
└── styles/
    ├── index.css                                          [追加 @import]
...
- 分别截图当前 `inset` preset + `classic` style 的首页（真实 id，不是 DESIGN 文件名 shadcn-classic.md）
...
**背景**：drift 类别（**需要先 verify 再修**，Plan v1 曾误把 "70 → 140" 作为 drift，实地 verify 后发现 CLAUDE.md 里根本没这说法——v2 修订清掉该臆造）：
1. preset 数量：旧文档写"inset / mix / top / side 四种布局"或"2 preset"，实际将变为 5 个
2. token 数量陈述：`ui-tokens/src/index.ts:97` 真相源是 `TOTAL_TOKENS = 70` / style。Plan A 新增 claude-warm 后 check:theme 要校验 3 style × 2 mode = 6 个 block 各 70 token（不涉及 70→140 改写）——**grep 确认 CLAUDE.md / AGENTS.md / specs 中所有 "70 token" 的措辞仍然正确**，不做改写
3. 密度档名：旧文档写"xs / default / lg"，改名后是 "compact / default / comfortable"
4. depcruise 规则数：旧文档写"7 条"，实际 9 条（见 Task 0 Step 4）
5. 默认 style / 默认 preset：`classic + inset` → `claude-warm + claude-inset`
6. style 数量：旧文档可能写"2 style"（classic + lark-console），更新为 "3 style"（+ claude-warm）
...
# 扫描范围（含 handoff，防止 drift 传染）
SCAN='CLAUDE.md AGENTS.md docs/specs/frontend/*.md docs/handoff/*.md'

# 类别 1：preset 数量
grep -nE "inset / mix / top / side|top/side preset|2 个 preset|2 preset|四种布局|四种 layout" $SCAN

# 类别 2：密度档名（xs/lg → compact/comfortable）
grep -nE "data-theme-scale[:=] ?['\"]?(xs|lg)['\"]?|scale: ?'(xs|lg)'|xs / default / lg|xs/default/lg" $SCAN

# 类别 3：depcruise 规则数 7 → 9
grep -nE "7 条 depcruise|7 条规则|7 (个 )?depcruise|7 dependency-cruiser|共 7 条" $SCAN

# 类别 4：默认 style / preset
grep -nE "默认.*(classic|shadcn)|defaultStyle.*(classic|shadcn)|defaultLayoutPreset.*inset|默认 preset.*inset" $SCAN

# 类别 5：style 数量
grep -nE "2 个 style|2 style|两个 style|2 套 (主题|style)" $SCAN
```

记录每一处位置。**特别检查** `docs/specs/frontend/02-ui-tokens-theme.md`（token 定义）、`05-app-shell.md`（preset 清单）、`09-customization-workflow.md`（使用者定制流程）、`docs/handoff/frontend-gap-analysis.md`（W1 指出含 "7 条规则" 字样）。

**不扫描 "70 semantic token"**——按概念 7 和 `ui-tokens/src/index.ts:97`，70 是 per-style 的正确数字（Plan v1 曾误把 "70 → 140" 作为 drift，属臆造）。

- [ ] **Step 2: 修正 CLAUDE.md**

找到 "前端依赖方向" 段落和 "技术栈" 段落里出现 preset / style / 密度档名 / depcruise 规则数的地方，改为：

- `inset / mix / top / side 四种布局` / `2 个 preset` → `inset / mix（M3）+ claude-classic / claude-inset / claude-rail（Plan A 新增），共 5 种`
- `2 个 style` / `2 style` → `3 个 style（classic + lark-console + claude-warm）`
- `xs / default / lg` → `compact / default / comfortable`
- `7 条 depcruise 规则` → `9 条 depcruise 规则`
- 默认 style `classic` → `claude-warm`；默认 preset `inset` → `claude-inset`

**不改** "70 semantic token" 相关表述——那是正确的 per-style 数字。
...
- [ ] **Step 2: 映射到 meta-build 的 70 个 core semantic token**

参考 `client/packages/ui-tokens/src/tokens/semantic-classic.css`（`--color-background / --color-foreground / --color-primary / ...`）。文件本身的行数可能多于 70（含注释 / 扩展变量），但 `TOKEN_NAMES` 白名单过滤后的 core 变量是 **70 / 每个 block**（由 `check:theme` 强制）。
...
- [ ] **Step 4: 补齐所有 70 个 core token（light + dark 各 70）**
...
Expected: 通过。check:theme 会校验 3 style × 2 ColorMode = 6 个 block，每 block 70 个 core token（以 `classic.light` 为基准，行数可大于 70 但白名单过滤后必须齐 70）。

- [ ] **Step 7: 提交**

```bash
git add client/packages/ui-tokens/src/tokens/semantic-claude-warm.css client/packages/ui-tokens/src/styles/index.css
git commit -m "feat(tokens): 新增 semantic-claude-warm.css（light + dark 各 70 个 core token）"
```
...
**Files:**
- Modify: `client/packages/ui-tokens/src/style-registry.ts`（追加 register 调用，同 `classic` / `lark-console` 注册相邻）
- Modify: `client/packages/app-shell/src/theme/style-provider.tsx`（`normalizeStyleId` 兜底从 `'classic'` 换为 `'claude-warm'`）
- Modify: `client/apps/web-admin/src/main.tsx`（`<StyleProvider defaultStyle="claude-warm">`）
- Modify: `client/packages/app-shell/src/i18n/zh-CN/shell.json`
- Modify: `client/packages/app-shell/src/i18n/en-US/shell.json`

**背景（概念 7 真相源对齐）**：
- 现有 style 注册位置是 `ui-tokens/src/style-registry.ts:103-119`（**不是** main.tsx）
- 注册 API 签名：`styleRegistry.register({ id, displayName, description, color: '#xxx', cssFile: './tokens/semantic-<id>.css' })`
- `color` 是纯 hex 字符串（不是 gradient），用于 Customizer 预览色块
- `cssFile` 是相对 `src/` 的路径（`./tokens/semantic-<id>.css`）——check:theme 会 `resolve(srcDir, cssFile)` 读取
- 默认 style **没有** `DEFAULT_STYLE_ID` 常量；由 `StyleProvider` 的 `defaultStyle` prop + `normalizeStyleId` 函数的 `'classic'` 兜底决定

- [ ] **Step 1: 在 style-registry.ts 末尾追加 claude-warm 注册**

```bash
# 先确认现有注册格式
sed -n '100,120p' client/packages/ui-tokens/src/style-registry.ts
```

在文件末尾（`lark-console` 的 `styleRegistry.register({...})` 之后）追加：

```ts
// 注册 claude-warm style（对齐 Anthropic Claude Design，Plan A v1 默认）
styleRegistry.register({
  id: 'claude-warm',
  displayName: 'Claude 暖色',
  description: '对齐 Anthropic Claude Design 的暖米白 + 暖橙品牌色',
  color: '#d97a3f',  // oklch(0.66 0.15 45) 的 sRGB 近似，用于 Customizer 色块预览
  cssFile: './tokens/semantic-claude-warm.css',
});
```

**关键**：
- 不要 import from main.tsx，不要用 `@mb/ui-tokens/...` 绝对路径
- `color` 是 sRGB hex（用于 Customizer 预览），不是 gradient / oklch
- 其他 style 的注册风格保持一致（参考 style-registry.ts:104-119）

- [ ] **Step 2: 修改默认 style 兜底为 claude-warm**

打开 `client/packages/app-shell/src/theme/style-provider.tsx`，找到 `normalizeStyleId` 函数（grep `normalizeStyleId|'classic'` 定位）：

```tsx
// 大致位置（具体行号先 grep 确认）
function normalizeStyleId(id: string | null | undefined): StyleId {
  if (id && styleRegistry.has(id)) return id;
  return 'classic';  // ← 兜底
}
```

把兜底 `'classic'` 改为 `'claude-warm'`。同时如果 `createDefaultState` 或其它地方有 hardcode `'classic'` 作为兜底，也一并审视（但 `classic` style 本身保留，只是默认走 claude-warm）。

- [ ] **Step 3: 修改 web-admin 的 StyleProvider defaultStyle**

打开 `client/apps/web-admin/src/main.tsx`，找到 `<StyleProvider>` 处（grep `StyleProvider` 定位）：

```tsx
// Before
<StyleProvider defaultStyle="classic" defaultColorMode="light">
// After
<StyleProvider defaultStyle="claude-warm" defaultColorMode="light">
```

若没有显式传 `defaultStyle`，追加该 prop（Provider 内部默认是 `'classic'`，Plan A 要显式 override）。

- [ ] **Step 4: i18n 新增文案**

- `client/packages/app-shell/src/i18n/zh-CN/shell.json`（theme.styles 段）：
  ```json
  { "theme": { "styles": { "claude-warm": "Claude 暖色" } } }
  ```
- `client/packages/app-shell/src/i18n/en-US/shell.json`：
  ```json
  { "theme": { "styles": { "claude-warm": "Claude Warm" } } }
  ```

（具体嵌套按现有 shell.json 的 theme 结构追加，不要新造顶级 key）
...
打开 localhost:5173：
1. 清空 localStorage（DevTools Application → Local Storage → Clear）后刷新
2. 默认落页应该走 claude-warm（整体暖米白 + 暖橙品牌色）
3. 点击 Topbar 调色板图标 → Customizer → Style 下拉看到 "Claude 暖色 / 经典 / 飞书控制台" 三项
4. 切换各 style 视觉立即变化
5. 切 dark mode 暖色变深棕
...
describe('claude-warm style registration', () => {
  ...
  it('becomes the fallback style in normalizeStyleId for unknown ids', () => {
    // normalizeStyleId 是 style-provider 内部函数；通过 StyleProvider 的 defaultStyle 行为间接验证
    // 或将 normalizeStyleId export 做直接单测（推荐）
    // 具体测试按现有 style-provider.test.tsx 风格
  });
});
```

- [ ] **Step 7: 跑 check:theme + 测试**

```bash
cd client && pnpm check:theme && pnpm -F @mb/app-shell test
```

Expected：
- `check:theme` 输出 "3 styles × 2 ColorMode = 6 blocks validated, 70 core tokens each" 或同等提示
- test PASS

- [ ] **Step 8: 提交**

```bash
git add client/packages/ui-tokens/src/style-registry.ts \
...
git commit -m "feat(style): 注册 claude-warm style 并设为默认（style-registry + StyleProvider defaultStyle + 兜底）"
```
...
`client/packages/app-shell/src/theme/style-provider.tsx` 顶部（grep `ThemeScale` 定位）：
...
- [ ] **Step 1.5: 同步 `mapLegacyThemeToState` 的 scale 值（W2 修复）**

同一文件 `mapLegacyThemeToState` 函数（第 85-93 行附近），把遗留的 `scale: 'xs'` 改为 `'compact'`：
...
若不改，新类型 `ThemeScale` 不含 `'xs'`，TS 编译报错 `"xs" is not assignable to ThemeScale`。

- [ ] **Step 2: 在 `readStateFromStorage` + `readStateFromDom` 两处加 migration（W3 修复）**

真相源 `style-provider.tsx:116-141`（`readStateFromStorage`）+ `143-163`（`readStateFromDom`）。**两处都要同步 migration**——只改 storage 不改 DOM 会导致 body 残留 `data-theme-scale='xs'` 在刷新瞬间被识别为非法值回落 default。

抽一个共享 helper 最干净：

```tsx
// style-provider.tsx 顶部 helpers 之间添加
...
```tsx
// readStateFromStorage（第 120 行附近）
const storedScaleRaw = window.localStorage.getItem(SCALE_KEY);
const storedScale = migrateScale(storedScaleRaw);
// 如果迁移发生，回写 storage
if (storedScale !== storedScaleRaw && storedScale) {
  window.localStorage.setItem(SCALE_KEY, storedScale);
}
// ...
scale: isThemeScale(storedScale) ? storedScale : 'default',

// readStateFromDom（第 150 行附近）
const attrScaleRaw = document.body.dataset.themeScale;
const attrScale = migrateScale(attrScaleRaw ?? null);
// 若发生迁移，立即更新 DOM（避免 CSS 仍然选中旧 attr）
if (attrScale !== attrScaleRaw && attrScale) {
  document.body.dataset.themeScale = attrScale;
}
// ...
scale: isThemeScale(attrScale) ? attrScale : 'default',
```

（具体插入位置的 before/after diff 在实施时用 Read 精确锚定行号。）
...
- [ ] **Step 6: 3 档密度数值对齐 Claude Design（保持 meta-build 现有 token 命名）**

**Token 命名策略（C2 决策 A）**：
- 保持 meta-build 现有 token 命名（`--size-control-height / --size-header-height / --size-sidebar-width / --size-sidebar-width-collapsed`）
- **不引入** Claude Design 的 `--row-h / --pad-x / --pad-y / --gap / --font-size-base`——那会造成两套并行命名无人消费
- Plan A 只对齐 **控件高度（--size-control-height）** 这一维的 3 档体感到 Claude Design 的 `--row-h`。其他维度（padding / gap / font-size）的 Claude Design 密度体感**不对齐**，视为 Plan B/C 之后的扩展（届时再扩展 `TOKEN_NAMES` 白名单 + 同步所有 L2 组件消费）

**Claude Design 真相源**（`/Users/ocean/Desktop/claude-design-test01/styles/tokens.css` 第 158-172 行）：
- compact: `--row-h: 36px` | default（:root）: `--row-h: 44px` | comfortable: `--row-h: 52px`

**meta-build 映射**（参考现有 `customizer.css` 行 1-15）：

```bash
# 先读现状
cat client/packages/ui-tokens/src/customizer.css
```

实际的 selector + 新值（覆盖 Step 5 已做的 selector 重命名之后）：

```css
[data-theme-scale='compact'] {
  --size-control-height: 2.25rem;        /* 36px，对齐 CD --row-h compact */
  --size-header-height: 2.75rem;          /* 44px，保持现有 xs 值 */
  --size-sidebar-width: 13rem;            /* 保持现有 xs 值 */
  --size-sidebar-width-collapsed: 3rem;   /* 保持现有 xs 值 */
}

[data-theme-scale='comfortable'] {
  --size-control-height: 3.25rem;         /* 52px，对齐 CD --row-h comfortable */
  --size-header-height: 4rem;             /* 保持现有 lg 值 */
  --size-sidebar-width: 18rem;            /* 保持现有 lg 值 */
  --size-sidebar-width-collapsed: 4.5rem; /* 保持现有 lg 值 */
}
```

（`:root` 的 default `--size-control-height` 保持不变——若现值不是 2.75rem/44px，考虑顺手对齐到 Claude Design default；但改 :root 会影响 default 态下所有组件，谨慎评估）

**注意**：`data-theme-scale='default'` 时不应写 attr（遵循现有约定：默认值不落 dataset，由 :root 默认 tokens 生效）。`style-provider.tsx` 已有这个逻辑（`if (state.scale !== 'default') body.dataset.themeScale = ...`）。

**已知不对齐项**（Plan A 不处理，标注以便未来跟进）：
- Claude Design 的 `--pad-x / --pad-y / --gap / --font-size-base` 密度体感未在 meta-build 落地
- Plan B/C 若发现某些业务场景视觉有差距（padding/字号在不同密度档感受不对），再决定是否扩展 `TOKEN_NAMES` 白名单
...
describe('ThemeScale localStorage migration', () => {
  ...
  it('removes data-theme-scale attr when switching back to default', () => {
    // W4 修复：不用未定义的 TestSwitcher，改用 useStyle hook 直接触发 setScale
    window.localStorage.setItem('mb_scale', 'compact');

    // 用一个内联测试组件通过 useStyle 切回 default
    function Switcher() {
      const { setScale } = useStyle();
      return <button onClick={() => setScale('default')}>to-default</button>;
    }
    render(<StyleProvider><Switcher /></StyleProvider>);

    // 初始 compact → dataset 被设
    expect(document.body.dataset.themeScale).toBe('compact');

    // 点击切回 default
    fireEvent.click(screen.getByText('to-default'));

    // default 按现有约定不写 attr（style-provider.tsx 的 `if (state.scale !== 'default') ...` 逻辑）
    expect(document.body.dataset.themeScale).toBeUndefined();
  });
});
```
...
## Task 12：claude-classic Preset（含 test-utils 共享 helper）

**Files:**
- **Create: `client/packages/app-shell/src/presets/__tests__/test-utils.tsx`（C5 修复：共享测试 helper，Task 13/14 复用）**
...
**视觉目标**：标准 topbar（56px 高，带品牌 + HeaderTab + 右侧功能区）+ 左侧 Sidebar（240px，按 activeModuleId 过滤）+ 主内容。对齐 Claude Design 的 `layout--classic`（参考 `/Users/ocean/Desktop/claude-design-test01/styles/app.css` 第 219-232 行 `.layout--classic` + `components/shell.jsx` 第 137-198 行 Sidebar 结构）。
...
**视觉风格纪律**：Plan A 目标是 **100% 还原 Claude Design**。preset 的 DOM 结构 / className 以 Claude Design 源码为准，**不使用** shadcn/ui 的 `Sidebar / SidebarProvider / SidebarInset` 复合组件——Claude Design 源是原生 `<aside>` + 数组 map 渲染。因此 Task 16 注册时 `claude-classic` 的 `supportedDimensions: []`（不支持 sidebarMode 维度，折叠 UI 由切到 `claude-rail` 表达）。

- [ ] **Step 0: 抽共享 renderWithShell helper（C5 修复）**

Task 12/13/14 三个 layout 测试都需要包裹 Router + i18n + QueryClient。抽到 `client/packages/app-shell/src/presets/__tests__/test-utils.tsx`：
...
import type { SystemItem, ShellLayoutProps } from '../../layouts/types';

// --- 共享夹具 ---
export const mkNode = (id: number, name: string, children: MenuNode[] = []): MenuNode => ({
  id, parentId: null, name, menuType: 'DIRECTORY',
  icon: null, sortOrder: id, visible: true, permissionCode: null, children,
});

export const mkChild = (id: number, parentId: number, name: string, permissionCode: string): MenuNode => ({
...
export const sampleMenuTree: MenuNode[] = [
  mkNode(1, '组织管理', [
    mkChild(10, 1, '成员与部门', 'iam:member:list'),
...
export const sampleResolveMenuHref = (n: MenuNode): string | null =>
  n.permissionCode ? (PATH_MAP[n.permissionCode] ?? null) : null;

/**
 * 包裹 Router + i18n + QueryClient，渲染指定 Layout。
 * Task 12/13/14 的 layout 测试共用。
 */
export function renderWithShell(
  initialPath: string,
  Layout: ComponentType<ShellLayoutProps>,
  overrides?: Partial<ShellLayoutProps>,
) {
  const rootRoute = createRootRoute({
    component: () => (
      <Layout
        ...
        resolveMenuHref={sampleResolveMenuHref}
        {...overrides}
      >
        <Outlet />
      </Layout>
...
**关键**：Task 13/14 的测试 `import { renderWithShell } from '../../__tests__/test-utils'`（相对 claude-inset/__tests__/xxx.test.tsx 的路径）。
...
import { isDisplayNode, getDisplayChildren } from '../../menu/menu-utils';

// 说明：
// - `resolveMenuIcon` 在 `menu/index.ts` re-export（✓）
// - `isDisplayNode` / `getDisplayChildren` / `findMenuPathByPath` 只能从 `menu/menu-utils` 直接导入
//   （`menu/index.ts` 没 re-export，参考概念 7 真相源表 + 现有 inset/mix preset 的 import 范式）
// - systems 来自 ShellLayoutProps.systems slot（Task 11 定义），由 L5 web-admin 注入
...
**Step 0 纪律（W6 修复）**：本 Task 的 **Step 1 注册** + **Step 3 改默认** 必须在同一个 commit 落地。模块加载时 `registry-core.ts` 的 `new LayoutRegistry('claude-inset')` 会立即执行，如果分 commit 中间态是"默认指向 claude-inset 但 claude-inset 未注册"，首次 `layoutRegistry.get(defaultId)` 会崩。Step 7 的单一 commit 已隐含，subagent 在中间 verify 时需保持完整性。
...
// 原因：Claude Design 的视觉形态差异通过"切 preset"表达（claude-classic/inset/rail 各自独立），
// 不在单个 preset 内部用 sidebarMode / contentLayout 维度来调整。这是 Claude Design 原生设计语言。
// meta-build 原 inset preset 支持 ['contentLayout', 'sidebarMode'] 是 shadcn Sidebar 复合组件带来的
// 能力，Plan A 新 preset 按 Claude Design 原生风格（原生 aside + 数组 map），不暴露这两个维度。
...
**Files:**
- Create: `apps/web-admin/e2e/layout-visual.spec.ts`（W9 修复：Playwright visual regression 前移）
- Create: `docs/handoff/claude-design-alignment/plan-a-regression.md`

- [ ] **Step 0: Playwright visual regression 自动 baseline（W9 修复：从 Task 19 前移）**

30 组合手动截图是纯粹的 drift 源——前移到本 Task 的 Step 0，用 Playwright `toHaveScreenshot()` 自动生成 baseline。后续 Plan B/C 和 PR 流程自动比对。

```ts
// apps/web-admin/e2e/layout-visual.spec.ts
...
// 先确认 localStorage key（来自真相源 style-provider.tsx 和 layout-preset-provider.tsx）：
// - STYLE_KEY = 'mb_style'
// - COLOR_MODE_KEY = 'mb_color_mode'
// - LAYOUT_PRESET_KEY = 'mb_layout_preset'（执行 Task 18 前先 grep 确认）
...
```bash
cd client && pnpm -F web-admin exec playwright test layout-visual --update-snapshots
```

生成 30 张 baseline PNG（存在 `apps/web-admin/e2e/layout-visual.spec.ts-snapshots/`）。commit 到 git，后续 CI 自动比对。

- [ ] **Step 1: 手动抽检补漏（非自动化场景）**

手动验证（Step 0 baseline 是静态截图，交互/动画/Customizer 对话框等抽检必要）。dev server 打开，依次切换：

| Style | Preset | 检查 |
|---|---|---|
| classic | inset | 原样现状，不崩 |
| classic | mix | 原样现状，不崩 |
| classic | claude-classic | 渲染不崩，视觉 OK |
| classic | claude-inset | 渲染不崩 |
| classic | claude-rail | 渲染不崩 |
...
- `check:theme`：新 style 的 token 有缺失（对照 semantic-classic 逐行补，应达 3 style × 2 mode × 70 core token）
...
- [ ] **Step 3: Visual regression 比对（对 Task 18 Step 0 生成的 baseline）**

```bash
cd client && pnpm -F web-admin exec playwright test layout-visual
```

Expected：30 screenshot 全部匹配 baseline（Plan A 范围内不应有视觉变化）。若失败说明 Plan A 在 Task 19 之前漏了某次视觉调整——检查 diff 并判断。
...
- L1 Token: semantic-claude-warm.css（light + dark 各 70 个 core token）+ claude-warm.md DESIGN
...
- [x] Critical 3（drift 扫描范围）→ Task 4 扩到 `docs/specs/frontend/02/05/09` 三份主 spec + `docs/handoff/*.md` + 扩展 grep 关键词（密度档名 / depcruise 7→9 / 默认 style / style 数量）
...
**2.5. Review v2 5 Critical 修复状态（code-reviewer 报告 @ ba632232 后补）**

- [x] C1 Token 数量口径错误 → 全 Plan 清理"70→140"臆造，改为正确陈述"每 style × 2 mode × 70 core token"。新增概念 7 锚定真相源 `ui-tokens/src/index.ts:97`
- [x] C2 密度 token 名错误 → Task 8 Step 6 保持 meta-build 现有 `--size-control-height` 等命名，只对齐控件高度数值；已知不对齐项（--pad-x / --gap / --font-size-base）诚实标注为 Plan B/C 扩展
- [x] C3 Style 注册位置 + cssFile + 默认机制 → Task 7 完全重写：注册在 `ui-tokens/src/style-registry.ts` 末尾；cssFile 格式 `./tokens/semantic-<id>.css`；默认走 `StyleProvider.defaultStyle` prop + `normalizeStyleId` 兜底
- [x] C4 menu 工具 import 路径 → Task 12 layout 改为 `resolveMenuIcon` from `'../../menu'` + `isDisplayNode / getDisplayChildren` from `'../../menu/menu-utils'`，对齐真相源和现有 inset/mix preset 范式
- [x] C5 renderWithShell 共享 helper → Task 12 Step 0 新建 `presets/__tests__/test-utils.tsx`，导出 `mkNode / mkChild / sampleMenuTree / sampleCurrentUser / sampleSystems / sampleResolveMenuHref / renderWithShell`，Task 13/14 复用

**3. Review v1 + v2 Warning/Observation 处理状态**
...
- [x] localStorage migration 单测 → Task 8 Step 7（3 个断言 + attr 回切用 useStyle hook 替代未定义的 TestSwitcher）
- [x] HeaderTabSwitcher 键盘导航 → Task 9 测试补 ArrowLeft/Right + tablist role + tabIndex 滚动
- [x] 30 组合手动截图太脆 → **Task 18 Step 0**（前移后）Playwright toHaveScreenshot 自动 baseline
...
- [x] W1 drift 扫描加 handoff 文件 → Task 4 Step 1 grep 范围含 `docs/handoff/*.md`
- [x] W2 LegacyTheme `'compact' → 'xs'` 映射 → Task 8 Step 1.5 追加 `mapLegacyThemeToState` 修复
- [x] W3 migration 插入点两处 → Task 8 Step 2 在 `readStateFromStorage` + `readStateFromDom` 各加 `migrateScale` helper
- [x] W4 未定义 TestSwitcher → Task 8 Step 7 最后一个单测改用 `useStyle` hook 内联 Switcher 组件
- [x] W6 Task 16 同 commit 纪律 → Task 16 Step 0 说明 Step 1 + Step 3 必须同 commit
- [x] W7 shadcn Sidebar vs Claude Design 原生 → **反向决策**：3 个新 preset 都用 Claude Design 原生 aside + 数组 map；Task 16 `supportedDimensions: []`（不暴露 sidebarMode/contentLayout 维度）
- [x] W9 Visual regression 前移 → Task 18 Step 0（从 Task 19 Step 3 移过来）
- [x] W10 Task 11.5 重复 Step 5 → 删除第二块
- [ ] W5 SystemSwitcherPopover ICON_MAP 和 menu icon resolver 不一致 → 未处理，Plan A 保持现状，未来如果 system icon 多了再考虑统一
- [ ] W8 systems 必选 vs optional → 未改，保持 optional（未注入时不渲染九宫格按钮）
- [ ] W11 mock 菜单顶层现状 → Task 0 Step 3 要求记录 baseline 含当前顶层数量
...
- Task 5 Step 4 有一个 TODO（补齐 70 core tokens × 2 mode），但附带了具体指令（diff 检查）
- 无其它未闭合占位
