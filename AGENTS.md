...
>
> **核心原则：索引而不是细节**。具体设计见 `docs/specs/`，决策原因见 `docs/adr/`，规划路线见 `docs/meta-build规划_v1_最终对齐.md`。AGENTS.md 不重复任何细节——**改架构修 specs，不修 AGENTS.md**，避免文档-代码 drift。
...
| Milestone | 内容 | 状态 |
|-----------|------|-----|
| **M0** | 文档对齐（规划 + backend specs + frontend specs + AGENTS.md） | ✅ 已定稿 |
| **M1** | 项目脚手架 + CI + 基础设施 | ✅ 已完成 |
| **M2** | 前端 L1 Style / ColorMode / Customizer 底座 + L2 原子组件库 | ✅ 已完成 |
...
| **M5** | notice canonical reference + SSE + 微信绑定 | ⚠️ notice canonical 已交付；order / approval 未启动；前端总收口未完成 |
...
**前端收口路线**：当前以前端 [frontend-gap-analysis.md](docs/handoff/frontend-gap-analysis.md) 的 P0-P3 为准：
`P0：质量线 + 真相收口` → `P1：Notice canonical 闭环` → `P2：UPMS 核心前端` → `P3：阶段性联调复盘`。
[m5-complete.md](docs/handoff/m5-complete.md) 只保留 M5 交付快照，不作为当前前端总状态入口。
...
│   │   ├── ui-primitives/           # L2 原子组件库（shadcn/ui v4 原版 + Sonner + Sidebar 等基础件）
│   │   ├── ui-patterns/             # L3 业务组件与页面骨架（Nx 系列 + PageHeader / StatusBadge / SearchInput）
...
AGENTS.md 不重复维护具体规则条目——`backend/README.md` 是后端的单一入口和反向索引，任何后端文档结构变化由 README.md 吸收，AGENTS.md 零感知。

<!-- 以下原 16 条 MUST NOT + 11 条 MUST 表已迁移到 backend/README.md 反向索引节，删除以保持 AGENTS.md 索引式结构 -->
...
AGENTS.md 不重复维护具体规则条目——`frontend/README.md` 是前端的单一入口和反向索引，任何前端文档结构变化由 README.md 吸收，AGENTS.md 零感知。
...
| 要加新 MUST/MUST NOT | 先在 backend/ 对应子文件加章节 + ArchUnit 规则，再补到 [backend/README.md 反向索引](docs/specs/backend/README.md#后端硬约束反向索引)；**AGENTS.md 不动**（已只索引 backend/README.md） |
...
| 要改 AGENTS.md | **只改顶层导航和指针**，不要塞架构细节（详情放 specs） |
...
# === 前端测试 ===
cd client && pnpm test                                    # 全量测试
cd client && pnpm -F @mb/ui-primitives test               # L2 测试
cd client && pnpm -F @mb/ui-patterns test                 # L3 测试
cd client && pnpm -F @mb/api-sdk test                     # api-sdk 测试

# === 前端质量检查（12 项，CI 等效）===
cd client && pnpm build                                   # 生产构建
cd client && pnpm check:types                             # TypeScript 类型检查（全包）
cd client && pnpm test                                    # 单元测试
cd client && pnpm check:theme                             # 主题完整性（按当前 token 合约校验）
...
| [docs/adr/](docs/adr/) | 架构决策记录（20 份） | 定稿 |
