...
│   ├── packages/
│   │   ├── ui-tokens/               # L1 Design tokens（54 token）+ Theme Registry（3 主题）
...
│   │   ├── app-shell/               # L4 布局（3 种）+ Provider 树 + i18n + 认证门面 + 菜单
...
项目级经验沉淀，每次 session 自动加载。完整索引、frontmatter 规范、生命周期管理见 [`.Codex/rules/INDEX.md`](.Codex/rules/INDEX.md)。
...
# === 后端 ===
cd server && mvn verify                                   # 全量构建 + 测试
cd server && mvn -Pcodegen generate-sources -pl mb-schema  # jOOQ codegen
cd server && mvn spring-boot:run -pl mb-admin             # 启动应用
cd server && mvn -pl mb-admin test -Dtest=ArchitectureTest # ArchUnit 测试
...
1. **文档-代码 drift**（nxboot AGENTS.md 描述了 7 个 shared/components 在磁盘是空目录）→ 本文档索引式 + 每章 verify 块
...
| [.Codex/rules/](.Codex/rules/) | **项目规则库**（pitfall + playbook，自动加载，持续积累） | 持续更新 |
